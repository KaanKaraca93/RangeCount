...
// Tema meta verisi (Alt_Sezon) IDM'den okunur ve nadiren değişir.
...
const normAltSezon = (v) => (v == null || v === '') ? null : String(v).trim().toUpperCase();

/**
 * Range Count Source V6.2 Service
 * V6 ile aynı mantık, tek fark:
 *   - Eşleştirme kriterinde FreeFieldThree yerine Alt_Sezon kullanılıyor.
...
class RangeCountSourceV6_2Service {
  ...
  /**
   * IDM'den tema PID'si için Alt_Sezon değerini çek (cache'li).
  ...
  async fetchAltSezon(themePid) {
    ...
    try {
      ...
      // Hata olursa eşleştirme durmasın; Alt_Sezon boş kabul edilir.
  ...
  /**
   * Verilen benzersiz tema PID'leri için Alt_Sezon'u sınırlı eşzamanlılıkla çeker.
   * @returns {Promise<Object>} pid -> altSezon (string|null)
   */
  ...
  /**
   * Colorway'lere altSezon bilgisini (IDM'den) iliştirir.
   */
  async annotateAltSezon(styles) {
    ...
    console.log(`🎨 V6.2 — ${pids.size} benzersiz tema için IDM Alt_Sezon çekiliyor...`);
    ...
    try {
      ...
      const params = {
        ...
        '$expand': [
          ...
          'UserDefinedField5',
          // Theme($select=Id,Description) eklendi — Description = IDM PID (Alt_Sezon için)
          'StyleColorways($select=StyleColorwayId,Code,Name,ColorwayUserField1,FreeFieldOne,FreeFieldFive,FreeFieldThree,ThemeId;$expand=ColorwayUserDefinedField4,ColorwayUserDefinedField5,Theme($select=Id,Description);$filter=ColorwayStatus ne 4)',
      ...
      console.log(`📞 V6.2 — PLM'den style verileri çekiliyor...`);
  ...
  matchPlaceholder(placeholder, style, colorway) {
    ...
    // FreeFieldThree yerine Alt_Sezon (tema üzerinden IDM'den çözüldü)
    if (normAltSezon(colorway.altSezon) !== normAltSezon(placeholder.Alt_Sezon)) return false;
    ...
    try {
      ...
      for (const placeholder of this.placeholders) {
          ...
          for (const colorway of style.StyleColorways) {
            ...
            if (this.matchPlaceholder(placeholder, style, colorway)) {
              matchedColorways.push({
                ...
                altSezon: colorway.altSezon || null,
        ...
        results.push({
          ...
          altSezon: placeholder.Alt_Sezon,
          // Plan vs Gerçekleşen
          planOptionSay: 1,
      ...
      console.error('❌ V6.2 eşleştirme hatası:', error.message);
      throw error;
    }
  }
...
module.exports = new RangeCountSourceV6_2Service();
